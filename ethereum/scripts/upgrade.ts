...
async function main() {
    ...
    const upgraded = await upgrades.upgradeProxy(aiToken.target, AIToken.connect(primaryDeployKey), { kind: 'uups' });
    console.log(`Upgraded: ${upgraded.target}`);
...
main()
    ...
    // to run: npx hardhat run .\scripts\upgrade.ts --network baseSepolia
